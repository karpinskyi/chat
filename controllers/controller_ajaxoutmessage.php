	...
	if (empty($this->data['0'])) {exit;}
		...
		if($this->data['0'] == 'message') {
			...
			if ($_SESSION['flowl'] == 'channel'){ 
				...
				include $this->teample . 'ajaxa.php';
				echo $messagerecordschannel;
			}
...
document.getElementById("passwch").value = "";
document.location.href ="' . SITE_PATH_BRAUZER . 'chat/channel/' . $readmessage->result['id'] . '";

';
				echo $recordschannelnew;
				
				/*echo 'document.getElementById("channels").value = "";';	
				echo 'document.getElementById("passwch").value = "";';				
				echo 'document.location.href ="' . SITE_PATH_BRAUZER . 'chat/channel/' . $readmessage->result['id'] . '";';*/
